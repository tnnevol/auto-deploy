...
const { execute } = require('../utils/shell.child.process')
...
const envConfigForDecrypted = (config, envConfig) => {
  return Object.assign(envConfig, {
    username: AES_decrypted(
      envConfig.username,
      config.cryptoKey,
      config.cryptoIv
    ),
    password: AES_decrypted(
      envConfig.password,
      config.cryptoKey,
      config.cryptoIv
    )
...
const execBuild = async (config, index) => {
  try {
    ...
    spinner.start()
    await execute(script, { cwd: process.cwd(), maxBuffer: maxBuffer })
    spinner.stop()
    succeed('打包成功')
  } catch (e) {
    error('打包失败')
...
const uploadLocalFile = async (config, index) => {
  try {
    ...
    log(
      `(${index}) 上传打包${underline(localPath)}至目录 ${underline(
        remoteFileName
      )}`
    )
...
module.exports = {
  description: '部署项目',
  apply: async (env, options = { startTask() {}, endTask() {} }) => {
    const { startTask, endTask } = options
    if (checkDeployConfigExists()) {
      ...
      if (env) {
        const envConfig = envConfigForDecrypted(config, createdEnvConfig(env))
        ...
        if (answers.confirm) {
          startTask && startTask([cluster])
          for (const env of cluster) {
            const envConfig = envConfigForDecrypted(
              config,
              createdEnvConfig(env)
            )
            createTaskList(envConfig)
